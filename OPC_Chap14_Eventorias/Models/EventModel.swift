...
struct EventModel: Identifiable, Equatable, Hashable {
    ...
    let image: String
    var imageUrl: URL?
    var profil: EventoriasUser?
    let type: EventType
    var canFetchEventImage: Bool = false
    

    init(id: UUID, name: String, description: String, date: Date, user: String, address: Address, image: String, imageUrl: URL? = nil, profil: EventoriasUser? = nil, type: EventType) {
        self.id = id
        self.name = name
